...
from utils import get_llama_tokenizer, count_llama_tokens, get_dirs_and_extensions, count_syntax_errors, sanitize_code_semantics, extract_functions, clean_llm_response, calculate_cost_oci, get_ts_tree_sitter_language_and_parser, get_func_id, get_type_aware_context
    ...
    try:
        ...
        for func_tuple in functions:
            ...
            if llm_response and isinstance(llm_response, dict) and "text" in llm_response:
                ...
                # --- LOG DA FUNÇÃO (MOVIDO PARA CIMA) ---
                # Agora regista SEMPRE, quer tenha erros de sintaxe, quer diga "No security issues found"
                ...
                # Se o LLM propôs uma modificação válida
                if cleaned_code != clean_func_text.strip():
                    ...
                    # Calcula os erros da nova árvore
                    current_errors = count_syntax_errors(parser.parse(bytes(temp_bytes)), language, parser)
                    ...
                    initial_errors = current_errors # <-- BUG RESOLVIDO: Atualiza o baseline para a próxima função!
